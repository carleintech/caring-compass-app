...
import type { User, ClientProfile, CaregiverProfile } from '@prisma/client'
...
export async function checkUserAccess(
  userId: string,
  resourceType: 'CLIENT' | 'CAREGIVER' | 'VISIT' | 'INVOICE',
  resourceId: string,
  user: User
): Promise<boolean> {
  ...
  switch (resourceType) {
    ...
    case 'VISIT':
      ...
      if (user.role === 'CLIENT' && visit.client.userId === user.id) return true
      if (user.role === 'CAREGIVER' && visit.caregiver?.userId === user.id) return true
      if (user.role === 'FAMILY') {
...
export async function findCaregiverMatches(
  ...
  for (const caregiver of caregivers) {
    ...
    const distance = calculateDistance(
      criteria.clientLat,
      criteria.clientLon,
      caregiver.latitude || 0,
      caregiver.longitude || 0
    )
    ...
    const availability = caregiver.availability.find(avail => 
